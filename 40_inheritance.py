...
class rbi(ubi):
    ...
    def total_cash(self):
        ...
        ubi._total += self._loan

    def loan_eligibility(self):
        if ((rbi._reserve - self._loan) < rbi._limit):
            return False
        else:
            rbi._reserve -= self._loan
            return rbi._reserve


_credit = input('Enter your loan amount:')
_cash = rbi(int(_credit))
...
_cash.total_cash()
print(f'Total cash with UBI now is Rs {ubi._total}')
